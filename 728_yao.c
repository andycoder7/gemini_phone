	...
	while (epoll_wait(epfd, &event, 1, -1) != -1)
	{
        ...
        //    if (!(events[i].events & EPOLLIN) )
...
#ifdef DEB_DETAIL
            printf("get new package, which seq = %d, firudp = %d, lastudp = %d\n", 
                    *(uint16_t*)(buff+7), firudp, lastudp);
#endif
            //lenth of data
            ...
            while(DATABUFF[lastudp][0] != 0 && DATABUFF[templastudp][0] != 0) {
                ...
                printf("DATABUFF[lastudp][0] = %d, DATABUFF[templastudp][0] = %d, seq0 = %d, seq1 = %d\n",
                        DATABUFF[lastudp][0], DATABUFF[templastudp][0], 
                        *(uint16_t*)(DATABUFF[lastudp]+7), *(uint16_t*)(DATABUFF[templastudp]+7));
#endif
...
#ifdef DEB_DETAIL
            printf("dealing package which seq = %d, firn = %d, lastn = %d\n",
                    seq, firn, lastn);
#endif
...
#ifdef DEBUG
                printf("get lost data package, seq: %d, and clear it %d\n", 
                        *(uint16_t*)(DATABUFF[firudp]+7), DATABUFF[firudp][0]);
#endif
            }
            else {
...
//                }
        ...
        {
#ifdef DEB_DETAIL
            printf("waitting recv data, firudp = %d, lastudp = %d, firn = %d, lastn = %d\n", 
                    firudp, lastudp, firn, lastn);
#endif
    ...
    while (1)
    {
        if(lastn != firn)
        {
            if(BIGBUFF[firn])
            {
                ...
                pthread_cond_signal(&WAIT_DEAL);
#ifdef DEB_DETAIL
                printf("upload package, which seq = %d, firn = %d, lastn = %d, and clear it %d\n",
                        *(uint16_t*)(&(BIGBUFF[firn][7])), firn, lastn, *(&(BIGBUFF[firn][0])));
#endif
                BIGBUFF[firn] = NULL;
            } else {
#ifdef DEBUG
