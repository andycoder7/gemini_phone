...
uint32_t tg_package_count   = 0;
uint32_t wifi_package_count = 0;
int lastn = 0;
int firn = 0;
...
int send_shunt(char * rate)
{
    ...
    data[0] = 2 + strlen(rate);
    data[1] = 0;
    data[2] = 11;

    strcpy(data+3, rate);
    sendto(tgfd, data, data[0] + 2, 0, (struct sockaddr *) &serveraddr, sizeof(serveraddr));
#ifdef DEBUG
        ...
        for(i = 0; i < epfds; i++) {
            ...
            printf("get lost package from fd %d, which seq = %d \n", events[i].data.fd, seq);
                ...
                if((seq>(MAX_SEQ-SEQ_TOR) && lastn<SEQ_TOR) || (lastn>(MAX_SEQ-SEQ_TOR) && seq<SEQ_TOR))
                    lastn = lastn>seq?seq:lastn; // get the little one
                else
                    lastn = lastn<seq?seq:lastn; // get the bigger one
            }
        } 
    }
#ifdef ERR
    ...
    while (1)
    {
        if(lastn != firn)
        {
            if(DATABUFF[firn][0])
            {
                ...
                DATABUFF[firn][0] = 0;
#ifdef DEB_DETAIL
                printf("upload package, which seq = %d, firn = %d, lastn = %d\n",
                        *(uint16_t*)(DATABUFF[firn]+7), firn, lastn );
#endif
            } else {
#ifdef DEBUG
                ...
                if(n<0) {
                    if(tg_package_count > 100 && wifi_package_count > 100) {
                        char rate[10] = {0};
#ifdef DEB_DETAIL
                        printf("tg_package_count %d, wifi_package_count %d\n", tg_package_count, wifi_package_count);
#endif
                        if(tg_package_count < wifi_package_count) {
                            rate[0] = '1';
                            rate[1] = ':';
                            int2str(rate+2, wifi_package_count/tg_package_count);
                        } else {
                            int t = int2str(rate, tg_package_count/wifi_package_count);
                            rate[t++] = ':';
                            rate[t++] = '1';
                            rate[t] = 0;
                        }
                        send_shunt(rate);
                        tg_package_count = wifi_package_count = 0;
                    }
#ifdef DEBUG
                    printf("lost it\n");
#endif
