...
void * recv_process(void *p)
{
	...
	while ((epfds = epoll_wait(epfd, events, 2, -1)) != -1)
	{
        for(i = 0; i < epfds; i++) {
            ...
            if((lasttg>(MAX_SEQ-SEQ_TOR) && lastwifi<SEQ_TOR) || (lastwifi>(MAX_SEQ-SEQ_TOR) && lasttg<SEQ_TOR))
                lastn = lastwifi<lasttg?lasttg:lastwifi; // get the bigger one
            else
                lastn = lastwifi>lasttg?lasttg:lastwifi; // get the litter one
...
#ifdef DEBUG
                printf("lost data package,firn=%d,lastn=%d\n",firn,lastn);
#endif
            }
            firn = (firn + 1) % MAX_SEQ;
        } else {
#ifdef DEBUG
                printf("waitting recv package,firn=%d,lastn=%d\n",firn,lastn);
#endif
